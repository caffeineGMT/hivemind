...
function dispatchEngineers(company, designSpecs) {
  ...
  const todoTasks = tasks.filter(t => {
    // Filter out project tasks and non-actionable statuses
    if (t.title.startsWith("[PROJECT]") || (t.status !== "backlog" && t.status !== "todo")) {
      return false;
    }
    // Check if dependencies are completed
    const { ready } = dependencies.areDependenciesCompleted(t.id);
    return ready;
  }).sort((a, b) => {
